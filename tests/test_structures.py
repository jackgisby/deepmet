...
class AuxiliaryTestCase(unittest.TestCase):
    ...
    def test_molecular_fingerprints(self):
        ...
        with open(
            self.to_test_results("sample_meta.csv"), "w", newline=""
        ) as sample_meta_file:

            sample_meta_csv = csv.writer(sample_meta_file)

            for smi, mol, fingerprint_sum in zip(
                self.test_smis, self.test_mols, test_fingerprint_sums
            ):
    ...
    def test_select_features(self):
        ...
        normal_meta_path, non_normal_meta_path = get_normal_non_normal_subsets(
            self.to_test_results(), normal_sample_size=50
        )

        # get fingerprints for sample molecules
        sample_fingerprints_path = get_fingerprints_from_meta(
            normal_meta_path, self.to_test_results("sample_fingerprints.csv")
        )
        ...
        sample_fingerprints_processed = pd.read_csv(
            sample_fingerprints_processed, header=None, index_col=False
        )
        ...
        sample_non_normal_fingerprints_processed = pd.read_csv(
            sample_non_normal_fingerprints_processed, header=None, index_col=False
        )

        self.assertEqual(sample_non_normal_fingerprints_processed.shape, (50, 2800))
        self.assertEqual(sample_non_normal_fingerprints_processed.iloc[0].sum(), 1431)
        ...
        for non_normal_input in non_normal_inputs.keys():
            ...
            (
                ...
                regenerated_non_normal_fingerprints,
                reselected_features,
            ) = select_features(
                normal_fingerprints_path=sample_fingerprints_path,
                normal_fingerprints_out_path=self.to_test_results(
                    "regenerated_fingerprints.csv"
                ),
                non_normal_fingerprints_paths=non_normal_inputs[non_normal_input],
                non_normal_fingerprints_out_paths=self.to_test_results(
                    "regenerated_non_normal_fingerprints.csv"
                ),
            )
            ...
            regenerated_fingerprints = pd.read_csv(
                regenerated_fingerprints, header=None, index_col=False
            )
            ...
            if non_normal_inputs[non_normal_input] is not None:
                ...
                regenerated_non_normal_fingerprints = pd.read_csv(
                    regenerated_non_normal_fingerprints[0], header=None, index_col=False
                )

                self.assertEqual(regenerated_non_normal_fingerprints.shape, (50, 2972))
                self.assertEqual(
                    regenerated_non_normal_fingerprints.iloc[0].sum(), 1451
                )
