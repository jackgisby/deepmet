...
class TrainModelTestCase(unittest.TestCase):
    ...
    def test_rescored_deepmet(self):
        ...
        self.assertEqual(
            [score_entry[0] for score_entry in self.deepmet_results_rescore],
            zinc_test_set,
        )
        ...
        original_scores = [
            score_entry[2]
            for score_entry in self.deepmet_model_fresh.results["test_scores"]
        ]
    ...
    def test_feature_processing(self):
        ...
        for fingerprint_csv in fingerprint_csvs:
            ...
            self.assertEqual(
                num_cols, 2746 if "processed" in fingerprint_csv else 13155
            )

    def test_train_with_modified_parameters(self):

        # train without non-normal class
        train_likeness_scorer(self.normal_meta_path, self.to_test_results())

        # train without filtering
        train_likeness_scorer(
            self.normal_meta_path, self.to_test_results(), filter_features=False
        )
    ...
    def test_train_single_model(self):
        ...
        dataset, dataset_labels, validation_dataset = load_training_dataset(
            normal_dataset_path=os.path.join(
                self.fresh_results_path, "normal_fingerprints_processed.csv"
            ),
            normal_meta_path=self.normal_meta_path,
            non_normal_dataset_path=os.path.join(
                self.fresh_results_path, "non_normal_fingerprints_processed.csv"
            ),
        ...
        train_single_model_scores = [
            score_entry[2]
            for score_entry in single_trained_model.results["test_scores"]
        ]

        # check that the re-loaded model gives the same test results as the original model
        for original_score, train_single_model_score in zip(
            original_scores, train_single_model_scores
        ):
...
class ScoreModelTestCase(unittest.TestCase):
    ...
    def test_pretrained_deepmet(self):
        ...
        calculated_scores = [
            score_entry[2] for score_entry in self.pretrained_deepmet_scores
        ]
        ...
        calculated_scores_no_model = [
            score_entry[2] for score_entry in self.pretrained_deepmet_scores_no_model
        ]

        zip_scores = zip(calculated_scores, calculated_scores_no_model)

        # check that the re-scores are the same as the scores for the original model
        for rescores, rescores_no_model in zip_scores:
            self.assertAlmostEqual(rescores_no_model, rescores, places=5)
    ...
    def test_feature_processing(self):
        ...
        fingerprint_csvs = (
            ...
            "input_fingerprints_processed.csv",
        )

        # the fingerprint sum of the first molecule for each fingerprint set
        fingerprint_first_row_sums = (2052, 1431)

        for fingerprint_csv, fingerprint_first_row_sum in zip(
            fingerprint_csvs, fingerprint_first_row_sums
        ):
            ...
            self.assertEqual(
                num_cols, 2800 if "processed" in fingerprint_csv else 13155
            )
