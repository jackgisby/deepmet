...
class DatasetTestCase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.normal_fingerprints_path = get_fingerprints_from_meta(
            cls.normal_meta_path, cls.to_test_results("normal_fingerprints.csv")
        )

        cls.non_normal_fingerprints_path = get_fingerprints_from_meta(
            cls.non_normal_meta_path, cls.to_test_results("non_normal_fingerprints.csv")
        )

    def test_unison_shuffled_copies(self):

        normal_meta = np.loadtxt(
            self.normal_meta_path, delimiter=",", dtype=str, comments=None
        )
        normal_fingerprints = np.loadtxt(
            self.normal_fingerprints_path, delimiter=",", comments=None
        )

        # shuffle the datasets
        random.seed(1)
        np.random.seed(1)
        shuffled_meta, shuffled_fingerprints = unison_shuffled_copies(
            normal_meta, normal_fingerprints
        )
        ...
        self.assertEqual(
            [r[0] for i, r in enumerate(shuffled_meta) if i < 5], hmdb_ids_to_check
        )
        ...
        for hmdb_id in hmdb_ids_to_check:
            ...
            self.assertTrue(
                (
                    normal_fingerprints[before_idx] == shuffled_fingerprints[after_idx]
                ).all()
            )
    ...
    def test_get_data_from_csv(self):
        ...
        random.seed(1)
        np.random.seed(1)
        acquired_data["standard"] = get_data_from_csv(
            self.normal_fingerprints_path, self.normal_meta_path
        )

        # without shuffling
        acquired_data["non_shuffled"] = get_data_from_csv(
            self.normal_fingerprints_path, self.normal_meta_path, shuffle=False
        )

        # add custom labels
        random.seed(1)
        np.random.seed(1)
        acquired_data["add_labels"] = get_data_from_csv(
            self.normal_fingerprints_path,
            self.normal_meta_path,
            add_labels=list(range(50)),
        )

        shuffled_hmdb_ids = ["HMDB0124840", "HMDB0128276", "HMDB0127036", "HMDB0007646"]
        non_shuffled_hmdb_ids = [
            "HMDB0003134",
            "HMDB0007646",
            "HMDB0007831",
            "HMDB0007891",
        ...
        for argument_type, (fingerprints, meta) in acquired_data.items():
            ...
            hmdb_ids = (
                non_shuffled_hmdb_ids
                if argument_type is "non_shuffled"
                else shuffled_hmdb_ids
            )
            self.assertTrue((meta[1:5, 0] == hmdb_ids).all())

            self.assertEqual(
                fingerprints[1:5].sum(),
                5281 if argument_type is "non_shuffled" else 8970,
            )
    ...
    def test_mol_key_dataset(self):
        ...
        random.seed(1)
        np.random.seed(1)
        fingerprints, meta = get_data_from_csv(
            self.normal_fingerprints_path, self.normal_meta_path
        )

        # inherits from LoadableDataset
        loadable_mol_key_dataset = LoadableMolKeyDataset(
            "./", fingerprints, test_idx=range(1, 5), train_idx=range(10, 15)
        )
        ...
        self.assertTrue(
            (
                train_loader.dataset.dataset.data == test_loader.dataset.dataset.data
            ).all()
        )
        ...
        # now try with labels
        loadable_mol_key_dataset_labelled = LoadableMolKeyDataset(
            "./",
            fingerprints,
        ...
        train_loader, test_loader = loadable_mol_key_dataset_labelled.loaders(
            batch_size=10
        )
    ...
    def test_load_training_dataset(self):
        ...
        self.assertTrue(
            (full_dataset.test_set.dataset.labels[range(50, 100)] == 1).all()
        )
    ...
    def test_load_testing_dataset(self):
        ...
        test_data, labels = load_testing_dataset(
            self.normal_fingerprints_path, self.normal_meta_path
        )
